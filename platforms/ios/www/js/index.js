//WorkWide-App-Phase3
var updateMapInterval=null;
var isOfflineAppWorking=false;
var arrayImgIds=[];
var isSelectedFromCamera=false;
var isSelectedFrom=3;
var isChangeImage=false;
var localAttachmentNameForUpdate=null;
var newCamera=true;
var attachmentUrl=null;
var attachmentId=null;
var newAttachmentData=null;
var attachmentPosition=null;
var AttachedDocId=null;
var AttachedImgData=null;
var offlineData="";
var isCallOnlineLandingPage=false;
...
function PushNotificationRegistration() {
	...
	push.on('registration', function(data) {
            //alert("NOTIFICATION ID : "+data.registrationId);
	...
	push.on('error', function(e) {
                        //alert("NOTIFICATION REG ERROR : "+e.message);
            });
}
	...
	switch(activePage.attr('id')) {
		case 'Landingpage':
            ...
            try{
               setTimeout(function() {
             if(updateMapInterval==null){
               updateMapInterval = setInterval(function(){ get_current_position(); }, 60000*2);
		...
		case 'attachmentPage':
			...
			break;
        case 'Attachment_Preview_Page':
              onShowAttachentPreview();
              break;
        case 'Attachment_Edit_Page':
               if(isEditOfflineData){
                 getAttachmentDocInfo(null, getAttachmentInfoId(getFileName(attachmentUrl)));
               }else{
...
function QuinticaWebService(requestType, methodName, param, callBack) {
    ...
    console.log('QuinticaWebService Params :'+JSON.stringify(param));
		...
		if(checkConnection()) {
                  ...
                  $.ajax({
                         ...
                         data: param,
                                 ...
                                 console.log('QuinticaWebService Response :'+ JSON.stringify(message));
                                 callBack(message);
...
function QuinticaWebServiceForBackgroundCall(requestType, methodName, param, callBack) {
    console.log('QuinticaWebService Url :'+webServiceUrl + methodName);
    console.log('QuinticaWebService Params :'+JSON.stringify(param));
    errorHandling(function() {
...
function updateUserLocation(position) {
    ...
    QuinticaWebServiceForBackgroundCall("POST", "getFSELocation", parameters, function(response) {
                       if(response.code == "1") {
...
//-------------------------------------------------
/*End of Location Update Functionality*/
//-------------------------------------------------
        ...
        if(response.code == "1") {
			...
			for(var i = 0; i < Object.keys(response.data).length; i++) {
				...
				if(response.data[i].priority == '1') {
					if(!critical) {
						$("#task_view_accepted").append("<hr  data-icon='false' style='background-color: #ff0000; height: 2px; width: 50%;'><h3 style='width: 100px; margin: auto;text-align: center; position: relative; padding-bottom: 15px;'>" + response.data[i].priority_type + "</h3>");
						critical = true;
					}
				}
				if(response.data[i].priority == '2') {
					if(!heigh) {
						$("#task_view_accepted").append("<hr data-icon='false' style='margin-top: 5%; background-color: #ff7e00; height: 2px; width: 50%;'><h3 style='padding-bottom: 15px; width: 100px; margin: auto;text-align: center; position: relative;'>" + response.data[i].priority_type + "</h3>");
						heigh = true;
					}
				}
				if(response.data[i].priority == '3') {
					if(!moderate) {
						$("#task_view_accepted").append("<hr data-icon='false' style='margin-top: 5%; background-color: #0c6410; height: 2px; width: 50%;'><h3 style='padding-bottom: 15px; width: 100px; margin: auto;text-align: center; position: relative;'>" + response.data[i].priority_type + "</h3>");
						moderate = true;
					}
				}
				if(response.data[i].priority == '4') {
					if(!low) {
						$("#task_view_accepted").append("<hr  data-icon='false' style='margin-top: 5%; background-color: #0836e2; height: 2px; width: 50%;'><h3 style='padding-bottom: 15px; width: 100px; margin: auto;text-align: center; position: relative;'>" + response.data[i].priority_type + "</h3>");
						low = true;
					}
				}
				if(response.data[i].priority == '5') {
					if(!planing) {
						$("#task_view_accepted").append("<hr data-icon='false' style='margin-top: 5%; background-color: #8808e2; height: 2px; width: 50%;'><h3 style='padding-bottom: 15px; width: 100px; margin: auto;text-align: center; position: relative;'>" + response.data[i].priority_type + "</h3>");
						planing = true;
					}
				}
				var count = 0;
	...
	QuinticaWebService("GET", "taskDetails", parameters, function(response) {
		if(response.code == "1") {
           ...
           try{
			...
			$(function() {
				var size = 100;
				var sizeAfterExpand = $('#View_page_for_Assigned_task').height() + 48;
				$('#View_page_for_Assigned_task ul').hide(0).parent();
				var sizeBeforeExpand = $('#View_page_for_Assigned_task').height() + 48;
        ...
        if(response.code == "1") {
			if(Object.keys(response.data).length == 0) {
				...
				for(k = 0; k < Object.keys(response.data).length; k++) {
					...
					if(response.data[k].priority == '1') {
						if(!critical) {
							$("#Accepted_Tasks_list").append("<hr  data-icon='false' style='background-color: #ff0000; height: 2px; width: 50%;'><h3 style='padding-bottom: 15px; width: 100px; margin: auto;text-align: center; position: relative;'>" + response.data[k].priority_type + "</h3>");
							critical = true;
						}
					}
					if(response.data[k].priority == '2') {
						if(!heigh) {
							$("#Accepted_Tasks_list").append("<hr data-icon='false' style='margin-top: 5%; background-color: #ff7e00  ; height: 2px; width: 50%;'><h3 style='padding-bottom: 15px; width: 100px; margin: auto;text-align: center; position: relative;'>" + response.data[k].priority_type + "</h3>");
							heigh = true;
						}
					}
					if(response.data[k].priority == '3') {
						if(!moderate) {
							$("#Accepted_Tasks_list").append("<hr data-icon='false' style='margin-top: 5%; background-color: #0c6410; height: 2px; width: 50%;'><h3 style='padding-bottom: 15px; width: 100px; margin: auto;text-align: center; position: relative;'>" + response.data[k].priority_type + "</h3>");
							moderate = true;
						}
					}
					if(response.data[k].priority == '4') {
						if(!low) {
							$("#Accepted_Tasks_list").append("<hr  data-icon='false' style='margin-top: 5%; background-color: #0836e2; height: 2px; width: 50%;'><h3 style='padding-bottom: 15px; width: 100px; margin: auto;text-align: center; position: relative;'>" + response.data[k].priority_type + "</h3>");
							low = true;
						}
					}
					if(response.data[k].priority == '5') {
						if(!planing) {
							$("#Accepted_Tasks_list").append("<hr data-icon='false' style='margin-top: 5%; background-color: #8808e2; height: 2px; width: 50%;'><h3 style='padding-bottom: 15px; width: 100px; margin: auto;text-align: center; position: relative;'>" + response.data[k].priority_type + "</h3>");
							planing = true;
						}
					}
	...
	QuinticaWebService("GET", "taskDetails", parameters, function(response) {
		if(response.code == "1") {
           ...
           try{
			...
			$(function() {
				var size = 100;
				var sizeAfterExpand = $('#View_page_for_Accepted_task').height() + 48;
				$('#View_page_for_Accepted_task ul').hide(0).parent();
				var sizeBeforeExpand = $('#View_page_for_Accepted_task').height() + 48;
	...
	QuinticaWebService("GET", "taskDetails", parameters, function(response) {
		if(response.code == "1") {
           ...
           try{
			...
			$(function() {
				var size = 100;
				var sizeAfterExpand = $('#View_page_End_trip_for_Accepted_task').height() + 48;
				$('#View_page_End_trip_for_Accepted_task ul').hide(0).parent();
				var sizeBeforeExpand = $('#View_page_End_trip_for_Accepted_task').height() + 48;
...
function Accepted_to_pending_task_list_reason() {
    ...
    var text_values = $("#pending_textarea_reason").val();
                                              
	if(select_vlaue == "" || select_vlaue == "-1" || select_vlaue == -1) {
...
function onSuccess_attach(imageURI) {
    ...
    if(isChangeImage){
        attachmentUrl = "data:image/jpeg;base64," +imageURI;
        var Attach_image = document.getElementById("image_change");
        Attach_image.src = attachmentUrl;
    }else{
        
        changePage("#Attachment_Preview_Page", "slide", false);
    }
}
 
 function onShowAttachentPreview()
        {
            try{
        ...
        for(var i=0;i<newAttachmentData.length;i++){
            if(newAttachmentData[i].option_type=="TEXT" || newAttachmentData[i].option_type=="NUMBER"){
                var Maxlen="";
            try{
                if(!isEmptyVal(newAttachmentData[i].type_limit))
                {
                    Maxlen="maxlength='"+newAttachmentData[i].type_limit+"'";
                }
            }catch(error){}
                $("#custom_fields").append("<input type='"+newAttachmentData[i].option_type+"'id='custom_field"+i+"' style='width:100%;' class='custom_field' placeholder='"+newAttachmentData[i].label+"' "+Maxlen+" ></input>");
            }else if(newAttachmentData[i].option_type=="TEXTAREA"){
                var Maxlen="";
               try{
               if(!isEmptyVal(newAttachmentData[i].type_limit))
                {
                ...
                $("#custom_fields").append("<textarea id='custom_field"+i+"'rows='4' style='width:100%;' class='custom_field' placeholder='"+newAttachmentData[i].label+"' "+Maxlen+" ></textarea>");
            }else if(newAttachmentData[i].option_type=="SELECT"){
                $("#custom_fields").append("<div><select name='select-choice-0' style='margin-top:10px; width:100%;' id='custom_field"+i+"'><option value='"+newAttachmentData[i].label+"'>"+newAttachmentData[i].label+"</option></select></div>");
                   try{
                      for(var k=0;k<newAttachmentData[i].type_values.length;k++){
                        var opt="<option value='"+newAttachmentData[i].type_values[k]+"' > "+ newAttachmentData[i].type_values[k] +"</option>";
                        $("#custom_field"+i).append(opt);
                    }
                }catch(error){}
            ...
            var Attach_image = document.getElementById("image_preview");
            Attach_image.src = "data:image/jpeg;base64," + Attach_imaged_file;
            if(isOfflineAppWorking){
    ...
    //-------------------------------------------------
    /*Start of Photo Location Functionality*/
    //-------------------------------------------------
    function getPhotoCaptureLocation() {
        navigator.geolocation.getCurrentPosition(setUserLocation);
    }
                                               
   
    function setUserLocation(position) {
        var locString="<br/>@ "+position.coords.latitude+"   "+position.coords.longitude;
        attachmentPosition=position;
        try{
        if(isSelectedFromCamera){
            $("#attachmentInfo").append(locString);
        }
        }catch(err){}
        }
    //-------------------------------------------------
    /*End of Photo Get Location Functionality*/
    //-------------------------------------------------

    //-------------------------------------------------
    /*Start Back From Attachment Preview Functionality*/
    //-------------------------------------------------
    function backFromAttachmentPreviewPage() {

        if(isOfflineAppWorking){
            changePage("#attachmentPage_offline", "slide", false);
        }else{
            changePage("#attachmentPage", "slide", false);
        }
    }
                                               
    //-------------------------------------------------
    /* End Back From Attachment Preview Functionality*/
    //-------------------------------------------------
...
//-------------------------------------------------
/* Start Attachment Edit Functionality*/
//-------------------------------------------------

function getUploadedImageInfo() {
        var parameters = {
            TOKEN: localStorage.getItem("TOKEN"),
            task_id: localStorage.getItem("Accepted_Task_id"),
            image_id: attachmentId
        };
        QuinticaWebService("POST", "getAttachmentImageInfo", parameters, function(response) {
              if(response.code == "1" && response.data != null) {
                 isSelectedFrom=3;
                 newAttachmentData=response.data;
                 onShowEditAttachent();
              }else {
                    showAlert("Please try again..!!", "WorkWide");
                    changePage("#attachmentPage", "slide", false);
...
function onShowEditAttachent()
            ...
            if(newAttachmentData[i].option_type=="TEXT" || newAttachmentData[i].option_type=="NUMBER"){
              ...
              try{
               ...
               Maxlen="maxlength='"+newAttachmentData[i].type_limit+"'";
                ...
                $("#edit_custom_fields").append("<input type='"+newAttachmentData[i].option_type+"'id='custom_field"+i+"' style='width:100%;' class='custom_field' placeholder='"+newAttachmentData[i].label+"' value='"+newAttachmentData[i].value+"' "+Maxlen+" ></input>");
             ...
             try{
               ...
               Maxlen="maxlength='"+newAttachmentData[i].type_limit+"'";
                ...
                $("#edit_custom_fields").append("<textarea id='custom_field"+i+"'rows='4' style='width:100%;' class='custom_field' placeholder='"+newAttachmentData[i].label+"' "+Maxlen+" >"+newAttachmentData[i].value+"</textarea>");
              
            }else if(newAttachmentData[i].option_type=="SELECT"){
                $("#edit_custom_fields").append("<div><select name='select-choice-0' style='margin-top:10px; width:100%;' id='custom_field"+i+"'><option value='"+newAttachmentData[i].label+"'>"+newAttachmentData[i].label+"</option></select></div>");
               try{
               for(var k=0;k<newAttachmentData[i].type_values.length;k++){
                var opt="<option value='"+newAttachmentData[i].type_values[k]+"' > "+ newAttachmentData[i].type_values[k] +"</option>";
                $("#custom_field"+i).append(opt);
                   if(!isEmptyVal(newAttachmentData[i].value)){
                       $("#custom_field"+i).val(newAttachmentData[i].value);
                   }
              }
            }catch(error){}
            }else if(newAttachmentData[i].option_type=="CHECKBOX"){
                $("#edit_custom_fields").append("<fieldset style='margin-top:10px;' id='custom_field"+i+"' data-role='controlgroup'><legend>"+newAttachmentData[i].label+"</legend></fieldset>");
               try{
               for(var k=0;k<newAttachmentData[i].type_values.length;k++){
                    var isChecked='';
                    for(var n=0;n<newAttachmentData[i].value.length;n++){
                         if(newAttachmentData[i].type_values[k]==newAttachmentData[i].value[n]){
                                isChecked='checked';
                         }
    ...
    attachmentPosition=null;
    if(isOfflineAppWorking){
       localAttachmentNameForUpdate=getFileName(attachmentUrl);
       ...
       getImageFromServer(attachmentUrl, function(myBase64) {
            Attach_image.src=myBase64;
...
//-------------------------------------------------
/* End Attachment Edit Functionality*/
//-------------------------------------------------
function getImageFromServer(url, callback) {
...
function del(folderpath, filename, content, contentType) {
        ApploadingIcon();
        window.resolveLocalFileSystemURL(folderpath, function (dir) {
                                                                                
            dir.getFile(filename, {create: false}, function (fileEntry) {
            fileEntry.remove(function (file) {
            console.log(filename+ " file removed succesfully.");
            //var newFile= "attachment_" + localStorage.getItem("offline_Accepted_task_id") + "_" + new Date().getTime() + ".png";
            setTimeout(function() {
	...
	window.resolveLocalFileSystemURL(folderpath, function(dir) {
		...
		dir.getFile(filename, {
			...
			file.createWriter(function(fileWriter) {
				...
				showAlert('Unable to save file in path ' + folderpath, 'WorkWide');
			});
		});
	});
	hideLoadingIcon();
...
function Accepted_to_rejection_task_list_reason_confirm_box() {
                                               var main_comment_reject = $("#rejection_textarea_reason").val();
...
function Accepted_to_rejectionsss_task_list_reason() {
                                               var text_values = $("#rejection_textarea_reason").val();
...
function getAttachmentInfo(){
        ...
        for(var i=0;i<newAttachmentData.length;i++){
            var chkBoxes = [];
            var jsonObj={};
            if(newAttachmentData[i].required_status=="1"){
                if(newAttachmentData[i].option_type=="TEXT" ||newAttachmentData[i].option_type=="NUMBER" || newAttachmentData[i].option_type=="TEXTAREA"){
                                        
                        if(isEmptyVal($("#custom_field"+i).val())){
                            showAlert('Please enter valid '+newAttachmentData[i].label, 'WorkWide');
                            $( "#custom_field"+i ).focus();
                            return null;
                        }
                       if(newAttachmentData[i].option_type=="NUMBER"){
                        var numString=$("#custom_field"+i).val();
                        numString=$.trim(numString);
                       if(numString.length > newAttachmentData[i].type_limit){
                            showAlert('Please enter valid '+newAttachmentData[i].label, 'WorkWide');
                            $( "#custom_field"+i ).focus();
                            return null;
                       }
                 ...
                 if($("#custom_field"+i).val()==(newAttachmentData[i].label)){
                  showAlert('Please select '+newAttachmentData[i].label, 'WorkWide');
                  return null;
                 }
               }
        ...
        attachment_data.push(jsonObj);
           ...
           if($("#custom_field"+i).val()==(newAttachmentData[i].label)){
                jsonObj[newAttachmentData[i].id]= "";
      
            }else{
                jsonObj[newAttachmentData[i].id]= $("#custom_field"+i).val();
            }
            attachment_data.push(jsonObj);
        }else{
            jsonObj[newAttachmentData[i].id]= $("#custom_field"+i).val();
...
function sending_image_to_backend() {
    ...
    try{
        if(attachmentPosition!=null){
            lat= attachmentPosition.coords.latitude;
            lang= attachmentPosition.coords.longitude;
        }
    }catch(err){
        lat=0.0;
        lang=0.0;
	...
	if(checkConnection()) {
        ...
        if(isOfflineAppWorking){
            ...
            var filename = "attachment_" + localStorage.getItem("offline_Accepted_task_id") + "_" + new Date().getTime() + ".png";
            var attachmentInfoId=getAttachmentInfoId(filename);
            createTableAttachmentDocInfo(localStorage.getItem("offline_Accepted_task_id"), attachmentInfoId, JSON.stringify(attachmentInfo), lat, lang,isSelectedFrom, false);
		...
		var parameters = {
            ...
            upload_from_gallary: isSelectedFrom,
            offline_mode:"2"
...
//------------------- Start Update Attachment ---------------------------------

function update_image_to_backend() {
            var attachmentUpdatedInfo=getAttachmentInfo();
            if(attachmentUpdatedInfo==null)
                return;
           var lat=0.0;
           var lang=0.0;
           try{
            ...
            if(isOfflineAppWorking){
                    ...
                    var contentType = "image/png";
                    var folderName = cordova.file.documentsDirectory;
                    var filename = localAttachmentNameForUpdate;
                    var attachmentInfoId=getAttachmentInfoId(filename);
                    createTableAttachmentDocInfo(localStorage.getItem("offline_Accepted_task_id"), attachmentInfoId, JSON.stringify(attachmentUpdatedInfo), lat, lang,isSelectedFrom, true);
                    del(folderName, filename, myBase64, contentType);
            }else{
                    ...
                    var parameters = {
                        ...
                        attachmentInfo: attachmentUpdatedInfo,
                        customer_document: Attach_imaged_file,
                        upload_from_gallary: isSelectedFrom,
                        offline_mode:"2"
                                               
                    };
                    console.log('UPDATE DATA >>>>>>>>>>>>>>>>>>:'+JSON.stringify(parameters));
...
$("#pending_textarea_reason").on('change  input', function() {
			...
			if(len == 0) {
				...
				$.each(obj, function(key, value) {
					...
					$("#categoryListview").append("<li data-icon='myicon6'><a href='#' onclick='categoryFields1(" + key + ",\"" + value + "\");' class='wborderradius ui-nodisc-icon " + value1 + "'>" + value + "</a></li>");
                       console.log(key +" KEY CATEGORY VALUE "+ value1);
//                    if(apiFlag == 0) {
...
function categoryFieldsList(a) {
   ...
   try{
	...
	QuinticaWebService("GET", "updateTaskScreen", parameters, function(response) {
		if(response.code == "1") {
                                          console.log(catname +" DATA OF CATEGORY0 "+ catnamediv);
			if(response.data == null || response.data == "null" || response.data == "" || response.data.length == 0) {
				hideLoadingIcon();
			} else {
				ApploadingIcon();
				if(apiFlag == -1) {
                       console.log(catname +" DATA OF CATEGORY 1: "+ catnamediv);
					...
					hideLoadingIcon();
                       ...
                       try{
                            $("#" + catnamediv).remove();
                                          ...
                                          console.log(catname +" DATA OF CATEGORY2 "+ catnamediv);
...
function retaining_images_from_backend() {
    ...
    var addMore=4;
    var isAddMore=false;
    $(".attachementcontainer").empty();
    for(var k=0;k<3;k++){
        $(".attachementcontainer").append("<img id='attachment_file"+k+"' class='attachementcontainer_class' />");
            $("#attachment_file"+k).click(function() {
            isChangeImage=false;
            Attachement_cameraphoto();
        });
        }
        $(".attachementcontainer_class").attr("src", "img/picture.png");
	var parameters = {
		TOKEN: localStorage.getItem("TOKEN"),
		task_id: localStorage.getItem("Accepted_Task_id")
	};
	QuinticaWebService("GET", "updateTaskCustomerDocumentCount", parameters, function(response) {
		if(response.code == "1") {
            newAttachmentData=response.attachment;
                       
			var imageData = response.count;
			if(imageData.length == 0 || imageData.length == "" || imageData.length == "0") {
				hideLoadingIcon()
				$(".attachementcontainer_class").attr("src", "img/picture.png");
				attachment_count = 0;
			} else {
				...
				for(var i = 0; i < imageData.length; i++) {
                       ...
                       if(i>noOfAttachment){
                           $(".attachementcontainer").append("<img id='attachment_file"+i+"' class='attachementcontainer_class' />");
                       addMore=1+i;
                       }else{
                        $("#attachment_file" + i).off("click");
                       }
					$("#attachment_file" + i).css("display", "block");
					$(".attachementcontainer").css("height", 47 * (1+i));
					var attachment_file = document.getElementById("attachment_file" + i);
					attachment_file.src = webServiceUrl + "customerDocumentImage/TOKEN/" + localStorage.getItem("TOKEN") + "/task_id/" + localStorage.getItem("Accepted_Task_id") + "/count/" + imageData[i].count + "";
					attachment_count = imageData.length;
                       
                       $("#attachment_file" + i).click(function(){
                            attachmentUrl=null;
                            var position=$(this).index();
                            attachmentUrl= webServiceUrl + "customerDocumentImage/TOKEN/" + localStorage.getItem("TOKEN") + "/task_id/" + localStorage.getItem("Accepted_Task_id") + "/count/" + imageData[position].count + "";
                            changePage("#Attachment_Edit_Page", "slide", false);
                            attachmentId=imageData[position].id;
                        });
                 if(i>=2)
                       {
                       isAddMore=true;
     ...
     // adding view for add more attachment
    //addMore=addMore+1;
    $(".attachementcontainer").append("<img id='addMoreAttachment' class='attachementcontainer_class' />");
    $("#addMoreAttachment").attr("src", "img/plus-2.png");
    $("#addMoreAttachment").click(function() {
            //$(".attachementcontainer").append("<img id='attachment_file"+addMore+"' class='attachementcontainer_class' />");
            //$( "#attachment_file" +addMore ).insertBefore( "#addMoreAttachment" );
            //$("#attachment_file"+addMore).click(function() {
                                  if(isAddMore){
                                    isChangeImage=false;
                                    Attachement_cameraphoto();
                                  }else{
            ...
            //});
            //$("#attachment_file" +addMore).attr("src", "img/picture.png");
        //addMore=addMore+1;
        //$(".attachementcontainer_class").attr("src", "img/camera.png");
        });
	hideLoadingIcon()
        ...
        if(response.data.length == 0) {
            ...
            try{
                ...
                if(count == count_value - 1) {
                    ...
                    if(keyss == "category_detail" || keyss == "category_data" || keyss == "OnholdDetails" || keyss == "rejectDetails" || keyss == "attachment_info") {
...
function listDir(path) {
    ...
    var noOfAttachment=3;
    var addMore=4;
    var isAddMore=false;
    $(".offline_attachementcontainer").empty();
    for(var k=1;k<=3;k++){
        $(".offline_attachementcontainer").append("<img id='attachment_file_offline"+k+"' class='attachementcontainer_class' />");
        $("#attachment_file_offline"+k).click(function() {
        isChangeImage=false;
          try{
                ...
                for(var i = 0; i < len; i++) {
                    ...
                    if(entries[i].isFile == "true" || entries[i].isFile == true) {
                        ...
                        if(n == "true" || n == true) {
                           arrayImgIds.push(i);
                            len1++;
                           if(len1>noOfAttachment){
                                $(".offline_attachementcontainer").append("<img id='attachment_file_offline"+len1+"' class='attachementcontainer_class' />");
                                addMore=len1;
                           }else{
                                $("#attachment_file_offline"+len1).off("click");
                           }
                            $("#attachment_file_offline" + len1).css("display", "block");
                            $(".offline_attachementcontainer").css("height", 47 * len1);
                           ...
                           $("#attachment_file_offline"+len1).click(function(){
                                newAttachmentData=[];
                                attachmentUrl=null;
                                var position=$(this).index();
                                isEditOfflineData=true;
                                attachmentUrl= entries[arrayImgIds[position]].nativeURL;
                                changePage("#Attachment_Edit_Page", "slide", false);
                            });
    ...
    $(".offline_attachementcontainer").append("<img id='addMoreAttachment' class='attachementcontainer_class' />");
    $("#addMoreAttachment").attr("src", "img/plus-2.png");
    $("#addMoreAttachment").click(function() {
    //$(".attachementcontainer").append("<img id='attachment_file"+addMore+"' class='attachementcontainer_class' />");
    //$( "#attachment_file" +addMore ).insertBefore( "#addMoreAttachment" );
...
function count_for_Resolved(tx, results) {
	var len1 = results.rows.length;
                                                                         console.log("MAIN TAIBLE DATA : "+ results );
...
function offline_Assigned_page_success_Workwide(tx) {
	tx.executeSql('SELECT * FROM Main_Database_Table WHERE status_id ="1" ORDER BY priority ASC', [], Data_list_for_Assigned, error_Workwide);
}
...
function Data_list_for_Assigned(tx, results) {
	...
	for(var i = 0; i < len; i++) {
		...
		if(results.rows.item(i).priority == '1') {
			if(!critical) {
				$("#offline_Assigned_page").append("<hr  data-icon='false' style='background-color: #ff0000; height: 2px; width: 50%;'><h3 style='width: 100px; margin: auto;text-align: center; position: relative; padding-bottom: 15px;'>" + results.rows.item(i).priority_type + "</h3>");
				critical = true;
			}
		}
		if(results.rows.item(i).priority == '2') {
			if(!heigh) {
                $("#offline_Assigned_page").append("<hr data-icon='false' style='margin-top: 5%; background-color: #ff7e00; height: 2px; width: 50%;'><h3 style='padding-bottom: 20px; width: 100px; margin: auto;text-align: center; position: relative;'>" + results.rows.item(i).priority_type + "</h3>");
				heigh = true;
			}
		}
		if(results.rows.item(i).priority == '3') {
			if(!moderate) {
				$("#offline_Assigned_page").append("<hr data-icon='false' style='margin-top: 5%; background-color: #0c6410; height: 2px; width: 50%;'><h3 style='padding-bottom: 15px; width: 100px; margin: auto;text-align: center; position: relative;'>" + results.rows.item(i).priority_type + "</h3>");
				moderate = true;
			}
		}
		if(results.rows.item(i).priority == '4') {
			if(!low) {
				$("#offline_Assigned_page").append("<hr  data-icon='false' style='margin-top: 5%; background-color: #0836e2; height: 2px; width: 50%;'><h3 style='padding-bottom: 15px; width: 100px; margin: auto;text-align: center; position: relative;'>" + results.rows.item(i).priority_type + "</h3>");
				low = true;
			}
		}
		if(results.rows.item(i).priority == '5') {
			if(!planing) {
				$("#offline_Assigned_page").append("<hr data-icon='false' style='margin-top: 5%; background-color: #8808e2; height: 2px; width: 50%;'><h3 style='padding-bottom: 15px; width: 100px; margin: auto;text-align: center; position: relative;'>" + results.rows.item(i).priority_type + "</h3>");
				planing = true;
			}
		}
		var count = 0;
...
function change_status_id(tx) {
	...
	if(inAcceptedmultiple == 0) {
		...
		tx.executeSql('UPDATE Main_Database_Table SET status_id = "5" , status_type = "In-progress" WHERE id = ' + localStorage.getItem("offline_Assigned_task_id") + '');
	} else {
		tx.executeSql('UPDATE Main_Database_Table SET status_id = "3" , status_type = "Accepted" WHERE id = ' + localStorage.getItem("offline_Assigned_task_id") + '');
	}
}
...
function offline_Accepted_page_success_Workwide(tx) {
	tx.executeSql('SELECT * FROM Main_Database_Table WHERE status_id = "3" ORDER BY priority ASC', [], data_list_for_Accepted, error_Workwide);
}
...
function data_list_for_Accepted(tx, results) {
	...
	for(var i = 0; i < len; i++) {
		...
		if(results.rows.item(i).priority == '1') {
			if(!critical) {
				$("#offline_Accepted_page").append("<hr  data-icon='false' style='background-color: #ff0000; height: 2px; width: 50%;'><h3 style='width: 100px; margin: auto;text-align: center; position: relative; padding-bottom: 15px;'>" + results.rows.item(i).priority_type + "</h3>");
				critical = true;
			}
		}
		if(results.rows.item(i).priority == '2') {
			if(!heigh) {
				$("#offline_Accepted_page").append("<hr data-icon='false' style='margin-top: 5%; background-color: #ff7e00; height: 2px; width: 50%;'><h3 style='padding-bottom: 15px; width: 100px; margin: auto;text-align: center; position: relative;'>" + results.rows.item(i).priority_type + "</h3>");
				heigh = true;
			}
		}
		if(results.rows.item(i).priority == '3') {
			if(!moderate) {
				$("#offline_Accepted_page").append("<hr data-icon='false' style='margin-top: 5%; background-color: #0c6410; height: 2px; width: 50%;'><h3 style='padding-bottom: 15px; width: 100px; margin: auto;text-align: center; position: relative;'>" + results.rows.item(i).priority_type + "</h3>");
				moderate = true;
			}
		}
		if(results.rows.item(i).priority == '4') {
			if(!low) {
				$("#offline_Accepted_page").append("<hr  data-icon='false' style='margin-top: 5%; background-color: #0836e2; height: 2px; width: 50%;'><h3 style='padding-bottom: 15px; width: 100px; margin: auto;text-align: center; position: relative;'>" + results.rows.item(i).priority_type + "</h3>");
				low = true;
			}
		}
		if(results.rows.item(i).priority == '5') {
			if(!planing) {
				$("#offline_Accepted_page").append("<hr data-icon='false' style='margin-top: 5%; background-color: #8808e2; height: 2px; width: 50%;'><h3 style='padding-bottom: 15px; width: 100px; margin: auto;text-align: center; position: relative;'>" + results.rows.item(i).priority_type + "</h3>");
				planing = true;
			}
		}
...
function change_status_type_from_accepted_to_inprogress_firstone(tx) {
	tx.executeSql('UPDATE Main_Database_Table SET status_id = "5" , status_type = "In-progress" WHERE id = ' + localStorage.getItem("offline_Accepted_task_id") + '');
...
function Going_Accepted_view_to_work_direct_page() {
	navigator.notification.confirm("Are you sure you want to end trip?", btnLabel_Going_Accepted_view_to_work_direct_page, "WorkWide", ["Yes", "No"]);
}
...
function _complete_data_for_offline_suceess(tx, results) {
	...
	for(i = 0; i < results.rows.length; i++) {
        ...
        delete OfflinTotalTasks[i]['attachment_info'];
...
function offlinetoonline(parameters) {
     try{
          // parameters= (JSON.stringi(parameters));
     
     }catch(error){console.log("ERROR FOR OFFLINESAVE : "+error.message)}
	QuinticaWebService("POST", "casorOfflineSave", parameters, function(response) {
		if(response.code == "1") {
			//changePage("#Landingpage", "slide", false);
		} else {
			console.log("Unable to connect..!!", "WorkWide");
			hideLoadingIcon2();
		}
	});
	hideLoadingIcon2();
...
function UploadAttachmentstoserver(path) {
 ...
 function getOfflineSavedImage(){
 
     var i=loopval;
     var len = imgEntries.length;
     if(i < len) {
 
       if(i==len-1){
         isCallOnlineLandingPage=true
       }
 
       console.log(imgEntries[i].isFile);
 
 
      if(imgEntries[i].isFile == "true" || imgEntries[i].isFile == true) {
          var str = imgEntries[i].nativeURL;
          if(str.indexOf(localStorage.getItem("offline_Accepted_task_id")) != -1) {
             var n = true;
          } else {
             var n = false;
         ...
         if(n == "true" || n == true) {
            var filepath = imgEntries[i].nativeURL;
            var attachmentDocuId=getAttachmentInfoId(getFileName(filepath));
            getFileContentAsBase64(filepath, function(base64Image) {
                        var base64 = base64Image.replace(/^data:image\/png;base64,/, "");
                        if(checkConnection()) {
                            isEditOfflineData=false;
                            getAttachmentDocInfo(base64, attachmentDocuId);
                        
                        } else {
                        console.log("Please Check Internet Connection", "WorkWide");
                        //hideLoadingIcon();
                        //return;
                        }
                        hideLoadingIcon();
...
$("#offline_reject_select_box").on('change select', function() {
	if($("#offline_reject_select_box").val() != "0") {
		$(".btngreycolor").removeClass("ui-disabled");
	}
});
$("#reject_textarea_reason_offline").on('change  input', function() {
...
function Going_Offline() {
    ...
    if(localStorage.getItem("OnDuty")=="0"){
        ...
        askToUserCurrentDutyStatus();
    }else{ navigator.notification.confirm("Confirm to go Offline?", btnLabel_Going_Offline_from_online, "WorkWide", ["Yes", "No"]);
    }
}
...
function _updating_in_progress_as_completed(tx) {
	...
	tx.executeSql('UPDATE Main_Database_Table SET status_id = "4" , status_type = "Resolved" WHERE id = ' + localStorage.getItem("offline_Accepted_task_id") + '');
	request_json = "";
	f = -1;
	datass = [];
	assets = [];
	...
	xhr.onload = function() {
		if(this.status == 200) {
			var blob = new Blob([this.response], {
                ...
                if(imageflag == 0){
                    saveFile(dirEntry, blob, "profile.png", imageflag);
                }
			else
                {
                    saveFile(dirEntry, blob, "entity.png", imageflag);
                }
		}
	};
	xhr.send();
...
function startTime() {
	...
	starttime = dt.getHours() + ":" + dt.getMinutes() + ":" + dt.getSeconds();
     try{
        offline_start_work_or_trip_to_change_task_status_id();
     }catch(err){}
	changePage("#offline_Accepted_entrip_view_page", "slide", false);
}
     
     function offline_start_work_or_trip_to_change_task_status_id() {
   
           var db = window.openDatabase("database_Workwide", "1.0", "database_Workwide_table", 2 * 1024 * 1024)
           db.transaction(change_task_status_id, error_Workwide);
     
     }
     
     function change_task_status_id(tx) {
...
/*==================================================================================================================*/
/*code for enable button */
$("#pending_select_reason_offline").on('change select', function() {
	if($("#pending_select_reason_offline").val() != "0") {
		$(".btngreycolor").removeClass("ui-disabled");
	}
});
$("#pending_textarea_reason_offline").on('change  input', function() {
...
function function_offline_status_change_in_inprogress_to_on_hold_success(tx) {
	...
	tx.executeSql('UPDATE Main_Database_Table SET status_id = "2" , status_type = "On hold" WHERE id = ' + localStorage.getItem("offline_Accepted_task_id") + '');
	offlineApiFlag = 0;
	cincrement = -1;
	changePage("#offline_Landingpage", "slide", false);
}
...
function getFileName(path) {
 ...
 function getAttachmentInfoId(imgName){
    console.log(" NAME OF ATTACHMENT: "+imgName);
     return imgName.split("_")[2].replace(".png","")
 }
                                                                         
 //----------------------------------- DATABASE FUNCTION ----------------------------------------
//----------------------------- Start Offline Attachment Information ---------------------------------
 function createTableAttachmentDocInfo(taskId, attachmentInfoId, attachmentDocInformation, latitude, langitude,isSelectedFrom, isUpdate){
     var db = window.openDatabase("database_Workwide", "1.0", "database_Workwide_table", 2 * 1024 * 1024)
     db.transaction(success_Workwide, error_Workwide);
 ...
 function getAttachmentDocInfo(imageData, attachmentDocId) {
    AttachedImgData=imageData;
    AttachedDocId=attachmentDocId;
    var db = window.openDatabase("database_Workwide", "1.0", "database_Workwide_table", 2 * 1024 * 1024)
    db.transaction(getAttachmentDocInfo_success_Workwide, error_Workwide);
 }
 
 function getAttachmentDocInfo_success_Workwide(tx) {
    tx.executeSql('SELECT * FROM AttachmentDocInfo WHERE attachment_doc_id =' + AttachedDocId + ' ', [], DataAttachmentDocInfo, error_Workwide);
 }
 
 function DataAttachmentDocInfo(tx, results) {
     if(isEditOfflineData){
      
       var DocDetail=JSON.parse((results.rows.item(0).attachment_doc_info));
       console.log("INFO OF ATTACHMENT : "+DocDetail);
       tx.executeSql('SELECT attachment_info FROM Main_Database_Table WHERE id =' + localStorage.getItem("offline_Accepted_task_id") + ' ', [], function TaskAttachmentInfo(tx, results) {
            newAttachmentData=JSON.parse((results.rows.item(0).attachment_info));
                     
                     for(var i = 0; i < newAttachmentData.length; i++) {
                
                               for(var k = 0; k < DocDetail.length; k++) {
                     
                     try{
                     var obj=DocDetail[k];       
                     $.each(obj, function(key, value) {
                           
                            if(JSON.stringify(key)==JSON.stringify(newAttachmentData[i].id)){
                               newAttachmentData[i]["value"]=value;
                             console.log(newAttachmentData[i].value +" INFO OF ATTACHMENT ER : "+value);
                            }
                            });
                     }catch(err)
                     {
                     console.log("INFO OF ATTACHMENT ERROR : "+err.message);
            ...
            console.log("INFO OF ATTACHMENT : "+JSON.stringify(newAttachmentData));
            onShowEditAttachent();
        }, error_Workwide);
...
//----------------------------- Start Offline Task Attachment Information ---------------------------------

 function getTaskAttachmentInfo() {
 
      var db = window.openDatabase("database_Workwide", "1.0", "database_Workwide_table", 2 * 1024 * 1024)
      db.transaction(getTaskAttachmentInfo_success_Workwide, error_Workwide);
 }
 
 function getTaskAttachmentInfo_success_Workwide(tx) {
    tx.executeSql('SELECT attachment_info FROM Main_Database_Table WHERE id =' + localStorage.getItem("offline_Accepted_task_id") + ' ', [], function DataTaskAttachmentInfo(tx, results) {
            newAttachmentData=JSON.parse(results.rows.item(0).attachment_info);
                  console.log("DATA OF ATTACHMENT : "+newAttachmentData);
    }, error_Workwide);
 }
 
//----------------------------- End Offline Task Attachment Information -----------------------------------
                                                                         
 //----------------------------- Start Delete Offline Task Attachment Information ---------------------------------
 
 function deleteTaskAttachmentInfo() {
 
      var db = window.openDatabase("database_Workwide", "1.0", "database_Workwide_table", 2 * 1024 * 1024)
      db.transaction(deleteTaskAttachmentInfo_success_Workwide, error_Workwide);
 }
 
 function deleteTaskAttachmentInfo_success_Workwide(tx) {
      tx.executeSql('DELETE FROM AttachmentDocInfo WHERE attachment_doc_id =' + AttachedDocId + ' ', [], function deleteofflineTaskAttachmentInfo(tx, results) {
            try{
                    var folderpath=cordova.file.documentsDirectory;
                    var filename="attachment_" + localStorage.getItem("offline_Accepted_task_id") + "_" + AttachedDocId + ".png";
                    ApploadingIcon();
                    window.resolveLocalFileSystemURL(folderpath, function (dir) {
